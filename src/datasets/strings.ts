export interface StringItem {
    label: string;
    value: any;
}

export const EMPTY_STRING: StringItem[] = [{ label: 'empty-string', value: '' }];
export const PADDED: StringItem[] = [
    { label: 'one-space', value: ' ' },
    { label: 'two-space', value: '  ' },
    { label: 'three-space', value: '   ' },
    { label: 'four-space', value: '    ' },
    { label: 'five-space', value: '     ' }
];


export const HTML_UNESCAPED_VALID: StringItem[] = [
    {
        label: 'example1',
        value: `<html>
                <head>
                    <title></title>
                </head>
                <body>
                    <b><p><div></div></p></b>
                </body>
            </html>`},
    {
        label: 'example2',
        value: `<html>
                <head>
                    <title></title>
                </head>
                <body>
                    <div> <input /> <button></button></div>
                </body>
            </html>`},
    {
        label: 'example3',
        value: `<html>
                <head>
                    <title></title>
                </head>
                <body>
                    <ul><li></li><li></li><li></li></ul>
                </body>
            </html>`},
    {
        label: 'example4',
        value: `<html>
                <head>
                    <title></title>
                </head>
                <body>
                    <div><a /></div>
                </body>
            </html>`}
];

export const HTML_UNESCAPED_INVALID: StringItem[] = [
    {
        label: 'example1',
        value: `<html>
                    <head>
                        <title></title>
                    </head>
                    <body>
                        <p><div><div><b></div></p></b>
                    </body>
                </html>`},
    {
        label: 'example2',
        value: `<html>
                <head>
                    <title></title>
                </head>
                <body>
                    <tr><td><td></td></tr>
                </body>
            </html>`},
    {
        label: 'example3',
        value: `<html>
                    <head>
                        <title></title>
                    </head>
                    <body>
                        <div><a></div>
                </html>`},
    {
        label: 'example4',
        value: `<html>
                    <head>
                    <body>
                        <div>
                            <h1><span><li></span></h1>
                    </body>
                </html>`}
];

export const ALPHANUMERIC_10: StringItem[] = [{
    label: '10',
    value: 'PXPrpSTs7g'
}];
export const ALPHANUMERIC_12: StringItem[] = [{
    label: '12',
    value: 'RkAAkf9IMIvP'
}];
export const ALPHANUMERIC_15: StringItem[] = [{
    label: '15',
    value: 'ZRvOQf2Jr7zw7ZW'
}];
export const ALPHANUMERIC_20: StringItem[] = [{
    label: '20',
    value: 'lfavHX2qFEV7StV1g3ki'
}];
export const ALPHANUMERIC_25: StringItem[] = [{
    label: '25',
    value: '7e5cJEczvmSig1N0QIYm81TBB'
}];
export const ALPHANUMERIC_30: StringItem[] = [{
    label: '30',
    value: 'WinjDkkKKydLFtgjn8tY7TMM4nwJw8'
}];
export const ALPHANUMERIC_40: StringItem[] = [{
    label: '40',
    value: 'Muf48UlZZJAxiS8C6pHR9KRBm8Lkwf34h5FU1dU5'
}];
export const ALPHANUMERIC_50: StringItem[] = [{
    label: '50',
    value: 'MGJANgqkS6G7iLPpHkl3lk3bULcg2yvZyQoUfPPxsDYdVO7eHW'
}];
export const ALPHANUMERIC_60: StringItem[] = [{
    label: '60',
    value: '076F6r6Z0wrRVdFu7Xq9nfCYKdHhB5Du0KrIADLstqCL9mBiGmfYe3KBISaQ'
}];
export const ALPHANUMERIC_70: StringItem[] = [{
    label: '70',
    value: '2ZPCgcgbGHEnodCReRNiVw7DsN8ME1vvaWCXbDYdFUhELP9QUWE1KnVP0SIU19wIf9Bp7q'
}];
export const ALPHANUMERIC_80: StringItem[] = [{
    label: '80',
    value: 'soAVmohU7rYdyNYjTaOkXQ1dIxdBMpxIh8kNzjH4BGLfo6PdzoiCtGN0BpgxVD5sVuTmL2UwkA8FSdjs'
}];
export const ALPHANUMERIC_90: StringItem[] = [{
    label: '90',
    value: 'gz9MRREcJEPfGFYYJbbomebZl43vMJIu4hJE9LpMaJ8jcud0dhmglRJKrVzsf5gSzTSNfqhoiZpajGBfnOGPvv5Tn8'
}];
export const ALPHANUMERIC_100: StringItem[] = [{
    label: '100',
    value: 'dz2E3xlMz3UDQWkOQG7OEGiId0bWLmHK0Zut559KtvrKK8toIhP9Kyp9Ls9AICuQrbrAoqCfC1JfnEiw8vNjFrfV5XQmC0ktl9h0'
}];
export const ALPHANUMERIC_120: StringItem[] = [{
    label: '120',
    value: 'BAHMOIbDYF8ieqo0heFwMJeGm2csQ5nCnXEplSao04TPe8Tw5B0tgh7nZzfDiSwGsi984fIgNb5ryw8IEkAMYqHkSnUhYYthFKZhTXAs1sCCk2nPTs8yqLJn'
}];
export const ALPHANUMERIC_150: StringItem[] = [{
    label: '150',
    value: 'IoBlY5BFoj6GDf8j30ZIgm38WTSK1OBXVksKp9gAulclQNpa16FSl4sW1oIjGoekyETNd4rukg6erGI7G2YdrCGvTRXi555VGCXgtBtwK77FInqD90sRK6Z7nyvimbhO91OHiXvzrhdPGkY7uNnZTZ'
}];
export const ALPHANUMERIC_200: StringItem[] = [{
    label: '200',
    value: 'dbXfEgiyWbJVWC4tdAD70WDPTdpoM5OkMF0TCBIflSNN4ynrkbMRgI2miVKEyAOnDLCipM3p5q8V0HshFwjppDGTCjBBRTmWKmGE9Qd3vVXiAGJUfmLrFZC3PTENWm1D6xHHV6IJdaVrVIMKJGFtyQYvysEISh4mMzIJx5lLhBg0RljBHVtcD5NMjss5LBdELUQP378P'
}];
export const ALPHANUMERIC_400: StringItem[] = [{
    label: '400',
    value: ''
}];
export const ALPHANUMERIC_1001: StringItem[] = [{
    label: '1001',
    value: ''
}];

export const EMOJI_SINGLE: StringItem[] = [
    { label: 'wink', value: ';-)' },
    { label: 'sad', value: ':-(' },
    { label: 'mask', value: ':mask:' },
    { label: 'joy', value: ':joy' },
    { label: 'smirk', value: ':smirk:' }
];

export const EMOJI_MULTI: StringItem[] = [
    { label: 'happy-set', value: ';-) ;-P :joy :smiley: :smile:' },
    { label: 'sad-set', value: ':-( :| ' },
    { label: 'charactor-set', value: ':boy: :older_man: :cop: :woman:' },
    { label: 'icons-set', value: ':sunny: :zap: :dog: :tiger: :ant:' }
];

export const JS_SNIPPETS_VALID: StringItem[] = [
    { label: 'example1', value: "str.toUpperCase()" },
    { label: 'example2', value: "let obj={1: 125, 2: 149.7};" },
    { label: 'example3', value: "alert('Welcome')" },
    { label: 'example4', value: "alert('Welcome')" }

];

export const JS_SNIPPETS_INVALID: StringItem[] = [
    { label: 'example1', value: "str.toUpperCase;" },
    { label: 'example2', value: "str.tolowercase();" },
    { label: 'example3', value: "let a={1=125, 2=149.7};" },
    { label: 'example4', value: "alert(((();" }
];
